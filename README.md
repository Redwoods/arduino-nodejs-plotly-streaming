...
Arduino code is also simple enough. The sampling frequency of pulse wave is 20 Hz when the delay is set to be 50 ms.
...
Node.js code (data_ppg.js) uses two npm modules, serialport and socket.io that are defined in package.json. When a PPG signal arrives from serial port, each PPG signal is sent to web clients with a nicely formatted date string generated by using a helper function provided by Plotly.
